...
let recordsCache = {}; // Local memory to avoid unnecessary fetching
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 3. Attach Event Listeners to all Student Circles
    document.querySelectorAll('.circle-btn').forEach(btn => {
        ...
        // Right Click = Subtract Point (Correction)
...
function toggleMode() {
    ...
    // Update counters to reflect the current mode
    updateAllCounters();
}
...
async function handleAction(studentId, action) {
    ...
    // 2. Modify value based on mode and action
    if (isExtraMode) {
    ...
    // 3. Instant UI Update (Optimistic rendering)
    recordsCache[studentId] = record;
    updateAllCounters();
    triggerAnimation(studentId, isAdd);

    // 4. Send to Supabase (Upsert handles both Insert and Update based on the unique constraint)
    const { error } = await supabase.from('class_records').upsert({
        student_id: studentId,
        record_date: currentDate,
...
function triggerAnimation(studentId, isAdd) {
    ...
    // 1. Floating Number Animation (+1 or -1)
    ...
    // 2. Sparkle Explosion (Only if Extra Points AND Adding)
